...
describe('filter test', () => {
	...
	test(`GIVEN there are multiple filters WHEN at least one item in array matches regex AND include was passed THEN item should NOT be filtered out`, () => {
		const result = filterXml([
			{key: 'category', regex: /^js$/g, include: true},
			{key: 'category', regex: /^html$/ig}
		]);
		expect(result.length).toBe(items.length - 1);
		expect(result.find(findCategory('js'))).not.toBeNull();
	});
});
